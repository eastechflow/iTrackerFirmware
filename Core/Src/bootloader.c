...
/**
  ...
  * @retval BL_ERR: upon failure
*/
uint8_t Bootloader_EraseBank(uint32_t Bank, uint32_t AppSizeInBytes)
{
	uint32_t NbrOfPages;
	uint32_t PageError  = 0;
	FLASH_EraseInitTypeDef  pEraseInit;
	...
	if ((AppSizeInBytes % FLASH_PAGE_SIZE)==0)
	{
		NbrOfPages = AppSizeInBytes / FLASH_PAGE_SIZE;
	}
	else
	{
		NbrOfPages = AppSizeInBytes / FLASH_PAGE_SIZE + 1;
	}

	if (NbrOfPages > FLASH_PAGE_NBPERBANK) NbrOfPages = FLASH_PAGE_NBPERBANK;
...
#if 0
uint32_t Bootloader_GetBankAddress(uint32_t Bank)
{
	uint32_t address;
	...
	{
		address = FLASH_BASE + (FLASH_PAGE_NBPERBANK * FLASH_PAGE_SIZE);
	}
...
/**
  ...
  * @retval BL_OK is returned in every case
*/
uint8_t Bootloader_FlashBeginBank(uint32_t Bank)
{
#if 0
    /* Reset flash destination address */
	if (Bank == FLASH_BANK_1)
	{
        flash_ptr = FLASH_BASE;
	}
	else
	{
		flash_ptr = FLASH_BASE + (FLASH_PAGE_NBPERBANK * FLASH_PAGE_SIZE);
	}
#endif
	// From RM0351 Reference Manual, page 91:
	// Note: When booting from bank 2, the boot loader will swap the Flash memory banks.
	// So...
	//    ...this address is always Bank 2
	flash_ptr = FLASH_BASE + (FLASH_PAGE_NBPERBANK * FLASH_PAGE_SIZE);
...
{
    if (appsize > (FLASH_PAGE_NBPERBANK * FLASH_PAGE_SIZE)) return BL_SIZE_ERROR;
    return BL_OK;
